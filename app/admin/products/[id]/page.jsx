...
export default function AdminProductEditPage() {
  ...
  useEffect(()=> {
    ...
    (async ()=> {
      try {
        ...
        const [pJson, bJson, cJson] = await Promise.all([pRes.json(), bRes.json(), cRes.json()]);
        if (!mounted) return;
        ...
        setCategoryIds((p.categoryIds || []).map(String));
        setDescriptionHtml(p.descriptionHtml || '');
        setMedia(Array.isArray(p.media) ? p.media.map(m => ({ url: m.url, alt: m.alt || '' })) : []);
        setSeo({ title: p.seo?.title || '', metaDescription: p.seo?.metaDescription || '' });
        setStatus(p.status || 'active');

        setVariants((p.variants && p.variants.length ? p.variants : []).map(v => ({
          sku: v.sku || '',
          options: v.options || {},
          price: {
            ...
            startAt: v.price?.startAt ? new Date(v.price.startAt) : null,
            endAt: v.price?.endAt ? new Date(v.price.endAt) : null,
          },
          weightGram: v.weightGram ?? null,
          dimensionsCm: { l: v.dimensionsCm?.l ?? null, w: v.dimensionsCm?.w ?? null, h: v.dimensionsCm?.h ?? null },
          barcode: v.barcode || '',
          status: v.status || 'active',
        })));

        setBrands(bJson.items || []);
  ...
  async function handleUpload(file) {
    ...
    try {
      ...
      setMedia(prev => [...prev, { url: uj.secure_url || uj.url, alt: '' }]);
    } catch (e) {
  ...
  async function handleSave(e) {
    ...
    try {
      const payload = {
        ...
        categoryIds,
        descriptionHtml,
        media: media.map(m => ({ url: m.url, alt: m.alt || '' })),
